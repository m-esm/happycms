...
namespace HCMS.Security.Infrastructer
{
    ...
    public class MyAuthorizeAttribute : AuthorizeAttribute
    {
        ...
        public override void OnAuthorization(AuthorizationContext filterContext)
        {
            ...
            var logonUrl = url.Content("~/" + currentCulture + "/manage/Auth/Login?returnUrl=" + HttpUtility.UrlEncode(filterContext.RequestContext.HttpContext.Request.Url.ToString()));
            ...
            if (!roles.Any(item => filterContext.HttpContext.User.IsInRole(item)))
            {
                ...
                filterContext.RequestContext.HttpContext.Response.Write("You are not authorized. redirecting to login page ...<br />" +
                    string.Format("<a href='{0}'>Login page</a>", logonUrl) +
                 "<script>setTimeout(function(){ window.location.href = '" + logonUrl + "'; },3000);</script>"
